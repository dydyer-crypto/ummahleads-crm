...
import { CheckCircle, ChevronDown, Check, ArrowLeft } from "lucide-react";
import {
  AUTH_LANGUAGES,
  useAuthLang,
  interpolate,
          ...
          {AUTH_LANGUAGES.map((l) => (
            <button
              ...
              style={{
                ...
                background: l.code === lang ? "rgba(212,175,55,0.08)" : "transparent",
              }}
            >
              <span className="text-base leading-none">{l.flag}</span>
              <span className="flex-1 text-start">{l.name}</span>
              {l.code === lang && <Check className="h-3.5 w-3.5 text-[#D4AF37]" />}
            </button>
          ))}
        </div>
      )}
        ...
        <Card className="w-full max-w-md border-slate-800 bg-slate-900/80 backdrop-blur">
          <CardHeader className="items-center pb-4 text-center">
            ...
            <p className="text-sm text-slate-400">
              {interpolate(t.checkEmailReset, { email })}
            </p>
          </CardHeader>
          <CardContent>
            <Link href="/login">
    ...
    >
      ...
      <Card className="relative z-10 w-full max-w-md border-slate-800 bg-slate-900/80 backdrop-blur">
        <CardHeader className="pb-4 text-center">
          <h1 className="text-xl font-semibold text-white">{t.resetPassword}</h1>
          <p className="text-sm text-slate-400">{t.resetSubtitle}</p>
        </CardHeader>
        <CardContent>
